...
const RSRC_KINDS = [
  ...
  { id: 'editor', label: 'Editor', types: ['eSTM', 'eBRS', 'MSta', 'FILT', 'LINF', 'DATA', 'PORT', 'RMAP', 'TMPL'],
    note: 'What the map editor kept in the data file: the stamps and brushes (their own gallery, under Composites), three saved game states (MSta: “Base”, “Plague Cured” and “Olpheltius Murdered”, which differ from Base at one byte each), the editor’s own tile names (DATA 260, a short list beside the file’s terrain table), three twelve-byte LINF records whose first two numbers read as a size (256 by 256, and 64 by 64 twice) though nothing here reads what they are for, the colour cycles (DATA 261), the record that says TxSt 999 is really a colour (RMAP), and the ResEdit templates. The displacement filters (FILT) are the game’s rather than the editor’s and have their own cards on this sheet. Of the rest, nothing in the game reads them, so what follows is this page’s guess: MSta holds the Char block’s sixty-four state bytes with the values second, so Plague Cured is quest value 2 at 10 and Olpheltius Murdered quest value 3 at 1; DATA 260 is an earlier tile-name list, six names different from 0xF004’s; DATA 261 is the five colour-cycling ramps as start, length and step, sixteen entries below where the program cycles them; DATA 263 is thirty-three prop tiles each with the word 5; DATA 269 is two wall brushes in a fifteen-entry layout; DATA 258 is a bitmap the world map’s size with 1,458 bits set in a pattern that repeats every thirty-two columns, and DATA 265 to 268 are empty bitmaps of the world’s and a 64 by 64 zone’s sizes; PORT 0 and 1, 413 and 2,351 bytes, have no shape this page could find.',
    view: 'RSRC' },
